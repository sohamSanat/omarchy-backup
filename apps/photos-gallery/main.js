...
app.setName('P-gallery');

let fingerprintProcess = null;
let isAppLocked = true;
let fpRestartTimer = null;

function stopFingerprintAuth() {
  if (fpRestartTimer) {
    clearTimeout(fpRestartTimer);
    fpRestartTimer = null;
  }
  isAppLocked = false;
  if (fingerprintProcess) {
    try {
      fingerprintProcess.kill('SIGTERM');
    } catch (e) {}
...
function startFingerprintAuth() {
  ...
  try {
    fingerprintProcess = spawn('fprintd-verify');

    fingerprintProcess.stdout.on('data', (chunk) => {
      const text = chunk.toString();
      console.log('[fprintd stdout]', text.trim());
      if (mainWindow && !mainWindow.isDestroyed()) {
        if (text.includes('verify-match')) {
          stopFingerprintAuth();
          mainWindow.webContents.send('auth-fingerprint-success');
        } else if (text.includes('verify-no-match')) {
          mainWindow.webContents.send('auth-fingerprint-status', 'Fingerprint not recognized. Try again or enter password.');
        } else if (text.includes('verify-retry-scan') || text.includes('swipe-too-short')) {
          mainWindow.webContents.send('auth-fingerprint-status', 'Fingerprint read error. Try again.');
        } else if (text.includes('Verify started') || text.includes('Verifying')) {
          mainWindow.webContents.send('auth-fingerprint-status', 'Touch the fingerprint sensor or enter your password');
        }
      }
    });

    fingerprintProcess.stderr.on('data', (chunk) => {
      console.log('[fprintd stderr]', chunk.toString().trim());
    });

    fingerprintProcess.on('close', (code) => {
      console.log('[fprintd close]', code);
      fingerprintProcess = null;
      if (code === 0 && mainWindow && !mainWindow.isDestroyed()) {
        stopFingerprintAuth();
        mainWindow.webContents.send('auth-fingerprint-success');
      } else if (isAppLocked) {
        // Re-arm fingerprint sensor after 1s if still locked
        fpRestartTimer = setTimeout(() => {
          if (isAppLocked) {
    ...
    console.error('Failed to spawn fprintd-verify:', err);
  }
}

function verifyUserPassword(password) {
  return new Promise((resolve) => {
    if (!password || typeof password !== 'string') return resolve(false);
    const username = os.userInfo().username;

    // Check password using su -c true (direct, clean pam_unix password verification)
    const suProc = spawn('su', ['-c', 'true', username]);
    let resolved = false;

    // Safety timeout in case su waits on tty
    const timer = setTimeout(() => {
      if (!resolved) {
        resolved = true;
        try { suProc.kill('SIGKILL'); } catch (e) {}
        resolve(false);
      }
    }, 6000);

    suProc.on('close', (code) => {
      if (resolved) return;
      resolved = true;
      clearTimeout(timer);
      resolve(code === 0);
    ...
    suProc.on('error', (err) => {
      console.error('su process error:', err);
      if (resolved) return;
      resolved = true;
      clearTimeout(timer);
      resolve(false);
...
const CONFIG_DIR = path.join(os.homedir(), '.config', 'omarchy-photos');
const CONFIG_FILE = path.join(CONFIG_DIR, 'config.json');
const ROOT_GALLERY_DIR = path.join(os.homedir(), 'Pictures', 'p-gallery-section');
...
function loadConfig() {
  ...
  const auntieDirect = path.join(picturesDir, '7. Secret class', 'milfs', 'auntie');
  const auntieInM = path.join(picturesDir, 'm', '7. Secret class', 'milfs', 'auntie');
  if (fs.existsSync(auntieDirect)) {
    initialFolder = auntieDirect;
  } else if (fs.existsSync(auntieInM)) {
    initialFolder = auntieInM;
  }
...
function parseArgv(argv) {
  for (let i = 1; i < argv.length; i++) {
    const arg = argv[i];
    if (!arg) continue;
    if (arg === '--home' || arg === '--modal' || arg === '--collapsed' || arg.startsWith('--album:') || arg.startsWith('--test-edit-cover:')) {
      return arg;
    }
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('get-initial-data', async () => {
    ...
    return {
      homeDir: os.homedir(),
      config,
      isTestUnlocked: process.env.TEST_UNLOCKED === '1',
  ...
  ipcMain.handle('auth-start-fingerprint', () => {
    startFingerprintAuth();
    return { success: true };
  });

  ipcMain.handle('auth-stop-fingerprint', () => {
    stopFingerprintAuth();
    return { success: true };
  });
