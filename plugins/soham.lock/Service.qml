...
Item {
  ...
  property int consecutiveFingerprintErrors: 0
  ...
  function resetAuthenticationState() {
    ...
    failureMessage = ""
    failedAttempts = 0
    consecutiveFingerprintErrors = 0
  ...
  function runWake() {
    ...
    if (lockRequested) {
      armBlankTimer()
      if (sessionLock.secure && fingerprintConfigured && !fingerprintPam.active && !fingerprintAuthenticating) {
        startFingerprint()
      }
    }
  }
  ...
  function startFingerprint() {
    ...
    if (consecutiveFingerprintErrors >= 3) {
      logEvent("fingerprint-throttled: consecutive errors limit")
      return
    }
    ...
    if (!fingerprintPam.start()) {
      fingerprintAuthenticating = false
      consecutiveFingerprintErrors += 1
      if (!displayBlanked && lockRequested && fingerprintConfigured && consecutiveFingerprintErrors < 3) {
        fingerprintRetryTimer.interval = 1500 * consecutiveFingerprintErrors
  ...
  function handleFingerprintFinished(result) {
    ...
    if (result === PamResult.Success) {
      consecutiveFingerprintErrors = 0
      finishUnlock()
    } else if (fingerprintConfigured && !displayBlanked) {
      fingerprintRetryTimer.interval = 1000
      fingerprintRetryTimer.restart()
    }
  }
  ...
  PamContext {
    ...
    onError: function(error) {
      ...
      root.consecutiveFingerprintErrors += 1
      root.logEvent("fingerprint-error: " + error)
      if (root.lockRequested && root.fingerprintConfigured && !root.displayBlanked && root.consecutiveFingerprintErrors < 3) {
        fingerprintRetryTimer.interval = 2000 * root.consecutiveFingerprintErrors
        fingerprintRetryTimer.restart()
      }
    }
  }
  ...
  Process {
    ...
    onExited: {
      ...
      root.fingerprintConfigured = configured
      if (root.lockRequested && configured && !root.displayBlanked) root.startFingerprint()
      else if (!configured && fingerprintPam.active) fingerprintPam.abort()
    }
  }
  ...
  IpcHandler {
    ...
    function status(): string {
      return JSON.stringify({
        ...
        consecutiveFingerprintErrors: root.consecutiveFingerprintErrors,
