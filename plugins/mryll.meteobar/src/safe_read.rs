...
//!
//! Every file this program reads — the Omarchy theme, the pywal palette, the
//! response cache — lives at a path anyone with write access to that directory
//! can replace. meteobar runs inside the long-lived, unsandboxed omarchy-shell
//! process, so a read that never returns or never stops growing does not fail
//! meteobar: it takes the shell with it.
//!
...
/// The response cache: one weather payload this program wrote. The largest a
/// run can ask for is `--days 7 --hours 24`, which clap caps, and that entry
/// measures a few KiB — so this sits an order of magnitude above the worst
/// legitimate case.
...
mod tests {
    ...
    fn tmp(name: &str) -> std::path::PathBuf {
        let d = std::env::temp_dir().join(format!("meteobar-safe-read-{name}"));
    ...
    fn an_oversize_file_reports_its_size_even_when_the_cap_cuts_a_codepoint() {
        ...
        assert!(e.to_string().contains("larger than 5 bytes"), "got: {e}");
    }
