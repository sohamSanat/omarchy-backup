...
Coding execution also uses an adaptive policy. `needle` is allowed only for narrow local edits and can deliver from focused deterministic checks without an independent reviewer. `standard` and `sword` keep the independent review requirement. UI always uses the standard or larger policy and requires rendered visual review.
