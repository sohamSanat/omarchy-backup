...
Item {
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      totalTokens: synced ? numberValue(stats.totalTokens) : numberValue(record.totalTokens),
      totalInputTokens: synced ? numberValue(stats.totalInputTokens) : numberValue(record.totalInputTokens),
      totalOutputTokens: synced ? numberValue(stats.totalOutputTokens) : numberValue(record.totalOutputTokens),
      dailyHistory: Array.isArray(record.dailyHistory) ? record.dailyHistory : [],
      allTimeStartDate: String(record.allTimeStartDate || ""),
