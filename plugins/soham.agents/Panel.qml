...
Panel {
  ...
  property bool showAllDailyHistory: false
  ...
  readonly property real historyTotalTokens: root.calculateTotalTokens(provider)
  readonly property real historyInputTokens: root.calculateInputTokens(provider)
  readonly property real historyOutputTokens: root.calculateOutputTokens(provider)
  ...
  function calculateTotalTokens(p) {
    if (!p) return 0
    if (p.totalTokens && Number(p.totalTokens) > 0) return Number(p.totalTokens)
    var usageMap = p.modelUsage || {}
    var total = 0
    for (var id in usageMap) {
      var bucket = usageMap[id] || {}
  ...
  function calculateInputTokens(p) {
    if (!p) return 0
    if (p.totalInputTokens && Number(p.totalInputTokens) > 0) return Number(p.totalInputTokens)
    var usageMap = p.modelUsage || {}
    var total = 0
    for (var id in usageMap) {
      var bucket = usageMap[id] || {}
  ...
  function calculateOutputTokens(p) {
    if (!p) return 0
    if (p.totalOutputTokens && Number(p.totalOutputTokens) > 0) return Number(p.totalOutputTokens)
    var usageMap = p.modelUsage || {}
    var total = 0
    for (var id in usageMap) {
      var bucket = usageMap[id] || {}
      total += Number(bucket.outputTokens || 0)
    }
    return total
  }
  ...
  function dayLabel(date, today) {
    if (today && !root.showAllDailyHistory) return "Today"
    var parsed = new Date(String(date || "") + "T00:00:00")
    if (isNaN(parsed.getTime())) return String(date || "")
    var name = dayName(date)
    if (root.showAllDailyHistory) {
      return (parsed.getMonth() + 1) + "/" + parsed.getDate() + " " + name
    }
  ...
  function dayTooltip(day, today) {
    ...
    if (day.inputTokens !== undefined && day.outputTokens !== undefined) {
      text += " (" + usage.formatTokenCount(Number(day.outputTokens || 0)) + " out, "
        + usage.formatTokenCount(Number(day.inputTokens || 0)) + " in)"
    }
    if (day.prompts || day.sessions) {
      text += " · " + Number(day.prompts || 0) + " prompts · "
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          Column {
            id: usageSection
            visible: !!root.provider && ((root.provider.recentDays && root.provider.recentDays.length > 0) || (root.provider.dailyHistory && root.provider.dailyHistory.length > 0))
            width: parent.width
            spacing: Style.spacing.md

            readonly property var historyList: (root.provider && root.provider.dailyHistory && root.provider.dailyHistory.length > 0)
              ? root.provider.dailyHistory
              : []
            readonly property var recentList: (root.provider && root.provider.recentDays)
              ? root.provider.recentDays
              : []
            readonly property var days: root.showAllDailyHistory && historyList.length > 0
              ? historyList
              : recentList
            readonly property real peak: root.daysPeak(days)
            ...
            Row {
              ...
              PanelSectionHeader {
                width: parent.width - (historyToggleBtn.visible ? historyToggleBtn.width + Style.spacing.sm : 0)
                text: root.showAllDailyHistory ? "TOKENS BY DAY (ALL HISTORY)" : "TOKENS BY DAY"
                foreground: root.foreground
                fontFamily: root.fontFamily
                anchors.verticalCenter: parent.verticalCenter
              }
              ...
              Button {
                id: historyToggleBtn
                visible: usageSection.historyList.length > 0
                text: root.showAllDailyHistory ? "7 days" : "All (" + usageSection.historyList.length + "d)"
                selected: root.showAllDailyHistory
                bordered: true
                foreground: root.foreground
                fontFamily: root.fontFamily
                ...
                onClicked: root.showAllDailyHistory = !root.showAllDailyHistory
              }
            }
          ...
          Column {
            id: historySection
            visible: !!root.provider && (root.historyTotalTokens > 0 || (root.provider.totalPrompts && root.provider.totalPrompts > 0))
            width: parent.width
            spacing: Style.spacing.md
            ...
            StatRow {
              ...
              value: usage.formatTokenCount(root.historyTotalTokens)
              highlight: true
              tooltip: Number(root.historyTotalTokens).toLocaleString() + " total tokens across history"
            }

            StatRow {
              visible: root.historyOutputTokens > 0
              width: historySection.width
              label: "Generated (output)"
              value: usage.formatTokenCount(root.historyOutputTokens)
              tooltip: Number(root.historyOutputTokens).toLocaleString() + " generated output tokens"
            }

            StatRow {
              visible: root.historyInputTokens > 0
              width: historySection.width
              label: "Prompts (input)"
              value: usage.formatTokenCount(root.historyInputTokens)
              tooltip: Number(root.historyInputTokens).toLocaleString() + " prompt input tokens"
            }

            StatRow {
              visible: !!root.provider && Number(root.provider.totalPrompts || 0) > 0
              width: historySection.width
              label: "Total prompts"
              value: String(Number(root.provider ? root.provider.totalPrompts : 0).toLocaleString())
              tooltip: "Total user prompts sent"
            }

            StatRow {
              visible: !!root.provider && Number(root.provider.totalSessions || 0) > 0
              width: historySection.width
              label: "Total sessions"
              value: String(Number(root.provider ? root.provider.totalSessions : 0).toLocaleString())
              tooltip: "Total unique sessions recorded"
            }

            StatRow {
              visible: !!root.provider && Number(root.provider.activeDays || 0) > 0
              width: historySection.width
              label: "Active days"
              value: String(root.provider ? root.provider.activeDays : 0) + " days"
              tooltip: (root.provider && root.provider.allTimeStartDate ? "Tracked since " + root.provider.allTimeStartDate + " · " : "") + "Every day is preserved permanently in SQLite ledger"
            }

            StatRow {
              visible: !!root.provider && String(root.provider.allTimeStartDate || "") !== ""
              width: historySection.width
              label: "Tracked since"
              value: String(root.provider ? root.provider.allTimeStartDate : "")
              tooltip: "Historical tracking starts from this date and persists forever in SQLite ledger"
            }
          }
  ...
  component DayRow: Item {
    ...
    Text {
      ...
      anchors.left: parent.left
      anchors.verticalCenter: parent.verticalCenter
      width: root.showAllDailyHistory ? Style.space(76) : Style.space(52)
    }
  ...
  component StatRow: Item {
    ...
    implicitHeight: Math.max(statLabel.implicitHeight, statValue.implicitHeight) + (highlight ? Style.spacing.md : Style.spacing.xs)

    Rectangle {
      visible: statRow.highlight
      anchors.fill: parent
      radius: Style.cornerRadius
    ...
    Text {
      ...
      textFormat: Text.PlainText
      text: statRow.label
      color: statRow.highlight ? root.foreground : root.dim
      font.family: root.fontFamily
      font.pixelSize: statRow.highlight ? Style.font.body : Style.font.caption
      font.bold: statRow.highlight
      anchors.left: parent.left
      anchors.leftMargin: statRow.highlight ? Style.space(10) : 0
      anchors.verticalCenter: parent.verticalCenter
    }
    ...
    Text {
      ...
      font.bold: true
      anchors.right: parent.right
      anchors.rightMargin: statRow.highlight ? Style.space(10) : 0
      anchors.verticalCenter: parent.verticalCenter
    }
    ...
    MouseArea {
      id: statHover
      anchors.fill: parent
      hoverEnabled: statRow.tooltip !== ""
      acceptedButtons: Qt.NoButton
    }

    PanelToolTip {
      visible: statHover.containsMouse && statRow.tooltip !== ""
