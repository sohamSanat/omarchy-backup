...
void main() {
    ...
    float tear = step(0.68, noise) * sign(hash(vec2(row, 7.0)) - 0.5);
    uv.x += tear * 0.010 * pulse;

    vec4 center = texture(tex, uv);
    vec2 chroma = vec2(0.0041 * pulse, 0.0);
    float red = texture(tex, uv + chroma).r;
    float blue = texture(tex, uv - chroma).b;
    vec3 color = mix(center.rgb, vec3(red, center.g, blue), 0.78 * pulse);
    float scanline = 0.020 * sin(uv.y * 900.0 + time * 40.0) * pulse;
